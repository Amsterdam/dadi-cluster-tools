...
if [[ -z $PRIMARY_DATE_KEY ]] ; then
    if [[ -z $SECONDARY_DATE_KEY ]] ; then
        ...
        kubectl create secret generic brp-keys --from-literal=$SECONDARY_DATE=$SECONDARY_KEY --from-literal=$PRIMARY_DATE=$PRIMARY_KEY --from-literal=brp_hash_array=$CONCAT_KEY --dry-run=client -o yaml | kubectl apply --server-side -f -
    else
        ...
        kubectl create secret generic brp-keys --from-literal=$SECONDARY_DATE=$SECONDARY_DATE_KEY --from-literal=$PRIMARY_DATE=$PRIMARY_KEY --from-literal=brp_hash_array=$CONCAT_KEY --dry-run=client -o yaml | kubectl apply --server-side -f -
    fi
else
    if [[ -z $SECONDARY_DATE_KEY ]] ; then
        ...
        kubectl create secret generic brp-keys --from-literal=$SECONDARY_DATE=$SECONDARY_KEY --from-literal=$PRIMARY_DATE=$PRIMARY_DATE_KEY --from-literal=brp_hash_array=$CONCAT_KEY --dry-run=client -o yaml | kubectl apply --server-side -f -
    else
...
kubectl create secret generic brp-keys --from-literal=$SECONDARY_DATE=$CURRENT_SECONDARY_DATE_KEY --from-literal=$PRIMARY_DATE=$CURRENT_PRIMARY_DATE_KEY --from-literal=brp_hash_array=$CURRENT_CONCAT_KEY --dry-run=client -o yaml | kubectl apply --server-side -f -